...
final class Localization: ObservableObject {
    ...
    static let table: [String: (String, String)] = [
        ...
        "Сохранять обложку": ("Preserve artwork", "Conservar portada"),
        "Оставлять встроенную обложку. Выкл — обложка удаляется из результата.": ("Keep embedded cover art. Off removes it from the result.", "Mantener la portada incrustada. Apagado la elimina del resultado."),
        "Сохранять структуру папок": ("Preserve folder structure", "Conservar estructura de carpetas"),
        "Воссоздавать дерево исходных папок в папке назначения вместо плоского списка.": ("Recreate the source folder tree in the destination instead of a flat list.", "Recrear el árbol de carpetas de origen en el destino en lugar de una lista plana."),
        "Оригинал": ("Original", "Original"),
        "Частота дискретизации выходного файла.": ("Sample rate of the output file.", "Frecuencia de muestreo del archivo de salida."),
        "Число каналов выходного файла.": ("Channel count of the output file.", "Número de canales del archivo de salida."),
        ...
        "Автоимпорт из папки": ("Auto-import from folder", "Autoimportar desde carpeta"),
        "Новые аудиофайлы из выбранной папки добавляются в библиотеку автоматически.": ("New audio files in the chosen folder are added to the library automatically.", "Los nuevos audios de la carpeta elegida se añaden a la biblioteca automáticamente."),
        "Папка": ("Folder", "Carpeta"),
        "Изменить…": ("Change…", "Cambiar…"),
